...
if (!filterArg && !countArg) {
  console.error('Please provide a filter argument');
  process.exit(1);
}
...
const filterData = (data, pattern) => {
  return data.map(location => {
    const filteredPeople = location.people.map(person => {
    ...
    return { ...location, people: filteredPeople };
...
const countEntities = (data) => {
  return data.map(location => {
    ...
    const newPeople = location.people.map(person => {
      const animalCount = person.animals.length;
      return { ...person, name: `${person.name} [${animalCount}]` };
    });
    return { ...location, name: `${location.name} [${peopleCount}]`, people: newPeople };
  });
};
...
if (filterArg) {
  result = filterData(data, filterPattern);
} else if (countArg) {
  result = countEntities(data);
}
